...
import "../styles/BuyerIntentForm.css";
...
import { buildMailtoLink } from "../utils/emailLink";
...
const BuyerIntentForm: React.FC = () => {
  ...
  const handleSubmit = () => {
    ...
    const url =
      contact.method === "whatsapp" ? buildWhatsAppLink(payload) : buildMailtoLink(payload);
  ...
  return (
    <div className="buyer-intent-form glass-card">
      ...
      {currentStep === "Intent" && <Step1 intent={intent} setIntent={setIntent} onNext={goNext} />}
      {currentStep === "Details" && (
        <>
          {intent === "personal" && (
            <Step2Personal
